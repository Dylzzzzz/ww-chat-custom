        ...
        >
            ...
            <div v-if="message.mentions && message.mentions.length > 0" class="ww-message-item__mentions">
                <MentionDropzone
                    ...
                    :key="`${mention.recordTypeId}-${mention.recordId}-${index}`"
                    :mention="mention"
                    :record-type-id="mention.recordTypeId"
                    :is-own-message="isOwnMessage"
                    :content="content"
                    :uid="`${uid}-mention-${index}`"
                />
            </div>
        </div>
    </div>
...
import MentionDropzone from './MentionDropzone.vue';
...
export default {
    ...
    setup(props, { emit }) {
        ...
        // Provide message data for mention dropzones
        provide('messageData', computed(() => props.message));
